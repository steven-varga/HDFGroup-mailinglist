...
namespace h5 {  namespace blaze {
        ...
        template <class Object, class T = typename meta::decay<Object>::type> using is_supported =
        ...
        // any non-cv qualified type is valid template parameter, based on conversation with
        // Klaus Iglberger -- author of blaze
        template<class T> struct is_valid : std::true_type{};
        template<class T> struct is_valid <const T> : std::false_type{};
        template<class T> struct is_valid <volatile T> : std::false_type{};
        template<class T> struct is_valid <const volatile T> : std::false_type{};
        template<class T> struct is_valid <T*> : std::false_type{};
}}
...
namespace h5 { namespace impl {
    ...
    template <class T> struct is_linalg<h5::blaze::rowvec<T>> : std::true_type {};
    template <class T> struct is_linalg<h5::blaze::colvec<T>> : std::true_type {};
    template <class T> struct is_linalg<h5::blaze::rowmat<T>> : std::true_type {};
    template <class T> struct is_linalg<h5::blaze::colmat<T>> : std::true_type {};

    // get read access to datastaore
    template <class Object, class T = typename meta::decay<Object>::type> inline
