...
namespace h5::meta {
    template <class T> using is_pod = std::conjunction<std::is_standard_layout<T>, std::is_trivial<T>>; // c++20 
    ...
    template <class T, class... Ts> struct is_contiguous : std::integral_constant<bool, is_pod<T>::value> {};
    ...
    template <class T, class... Ts> struct is_contiguous <std::vector<T,Ts...>> :
        std::integral_constant<bool, is_pod<T>::value>{};
