...
int main(){
	...
	std::cout << h5::dt_t<op::ldouble_t>() << std::endl;
	...
	{ //method 3, use STL 
		std::vector<op::ldouble_t> V(20);
		std::generate(V.begin(), V.end(), [&](){
			return static_cast<op::ldouble_t>(sample(random_float));
		});
		for( int i=0; i<V.size(); i++ )
			std::cout << static_cast<long double>( V[i] ) << " ";
		std::cout << std::endl;

		h5::write(fd, "opaque", V); // single shot write

		auto data = h5::read<std::vector<op::ldouble_t>>(fd, "opaque");
	...
	std::cout <<"\n\nsize of long double: " << sizeof(long double) << " sizeof ldouble_t: " << sizeof(op::ldouble_t) <<"\n";
}
